...
3、重写run方法的目的？
	每个线程都有自己的任务代码，主线程的任务代码是main方法中的所有代码，
	而自定义线程的任务代码就是Run方法中的代码
4、调用线程的start方法开启线程，一个线程一旦开启就会执行run中的所有代码
5、注意：run方法不能直接的调用，直接调用相当于调用一个普通的方法，
6、线程的生命周期状态图（非常重要）


网络编程：
	网页编程（以html语言作为基础实现浏览器与服务器之间的通信）
	网络编程：不需要通过浏览器进行通信
1、计算机网络：分布在不同地域的计算机通过外部设备把计算机链接起来，
达到了数据传输，共享资源的效果
2、计算机与计算机通信的三要素：
-ip地址
-端口号：门牌号
...
ip地址：
1、ip地址的本质是由32个二进制位数来组成的，为了方便人类记录，把Ip地址分为了4段，每个8位
2、ip地址由两个部分组成
3、ip地址=网络号+主机号
...
端口号：
1、范围0-65535、0-1023，不能由用户来使用

通信协议：
1、tcp
2、udp:不需要建立连接，比如qq的发送，数据包可能会丢失，游戏的数据传输
3、tcp协议：面向连接的，确定对方有人才发送数据，打电话，feiq的文件传输，效率低，属于可靠协议
4、java使用了一个类用于描述ip地址，对应有一个ip地址类

tcp的特点：
1、tcp协议通信是面向连接的，飞q的文件传输，打电话，tcp的客户端一旦建立，马上要与服务端建立连接
2、tcp在数据传输之前一定要建立连接
3、tcp是基于IO流进行数据传输的，在连接中传输大数据量
4、通过三次握手机制连接，可靠协议（保证数据传输的完整性）
5、因为tcp是面向连接的，所以效率稍低
6、tcp协议是分客户端和服务端的
7、为什么serversocket没有设置获取输入与输出的流
8、一个服务端是可以与多个客户端连接的
9、数据发送的端口不能由程序指定，是由CPU来指定的
10、一个网络程序如果能接收和发送数据，则至少要监听两个端口
11、浏览器与服务器之间的数据传输就是基于tcp协议传输的
12、写一个服务端与浏览器通信是使用tcp协议的，是传输协议
13、http协议是用于规范浏览器与服务器之间通信的数据的格式
14、使用字符流写出数据的时候，数据是写入了其内部的缓冲字符数组中，需要
调用flush方法

udp通讯协议：
1、java中不管是哪种通信协议，都统称为socket（插座，套接字）通信。
2、通信的双方都需要安装插座
3、在不同协议下就应该有不同的插座	
4、datagramsocket为udp协议下的服务类
5、将数据及其源和目的封装为数据包，不需要建立连接
6、因为无连接，故不可靠（会丢失数据包），但是速度快
7、udp协议不分客户端与服务端，只分发送端和接收端
8、在udp协议中有一个ip地址称作为广播ip地址，广播ip地址主机号为255的。
9、给广播ip地址发送消息的时候，在同一个ip段的都可以接收到消息
10、数据包在什么时候会丢失呢？
	带宽不足
	cpu处理能力不足的时候
	编写一个程序让数据包出现丢失的情况
...
java基础加强+复习：
1、jar文件双击允许经对于图形化界面的程序，对控制台程序不起作用
2、bat批处理文件：所谓的批处理文件就是一次性可以执行多个命令的文件
3、批处理文件如何编写
	新建一个记事本文件，把后缀改为.bat即可，然后把要执行的命令写在批处理文件上
4、bat处理文件常用的命令：
	pause;让控制台停留下来，不要一闪而过
	setclasspath命令只对文件夹、.jar、.zip文件有用
	echo off:执行该命令一下的命令都会隐藏起来，只显示效果
	@：隐藏当前行的命令
	echo:向控制台输出指定的语句
	title:设置控制台的标题
	color:设置控制台的颜色
...
单例设计模式：保证在内存中只存在一个对象

饿汉设计模式：
懒汉设计模式：存在线程安全问题	
1、私有化构造函数
2、声明本类的引用类型变量，但是不创建对象
3、提供一个公共静态的方法获取本类的对象，获取之前先判断是否创建了本类的对象
如果已经创建了，则直接返会，如果没有则先创建再返回

反射：（非常有用）
1、反射这门技术在左框架的开发的时候就非常有用了，做比较通用的工具类的开发的时候
2、当一个class文件被加载到内存中的时候，jvm马上对该class进行成员变量的解析，然后将class文件的成员数据封装到一个class对象中	 
，我们如果可以获取到Class对象，就可以通过该Class类来操作该类的所有成员
3、对象就是一堆数据的集合
4、Class对象的获取方法
5、推荐使用第一种
6、通过Class对象获取构造方法
7、注意：在反射技术中一个类的任何成员都使用了一个类描述
8、通过Class对象获取一个类成员变量

工厂设计模式：
1、就是专门用于产生对象的方法
2、设计一个工厂可以产生任何类型，产生的对象是根据配置文件而定
3、观察者设计模式：当一个事物发生了指定动作的时候，要通知另外一个事物做相应大的动作
4、程序之间讲究的是低耦合，高类聚，类与类之间尽量不要关联太多
5、步骤：
	把要通知对方的行为抽取出来定义为一个接口，然后再本类中维护该接口的成员。
6、断点调试：让程序运行到断点的时候停止运行，可以查看当前变量或者对象的数据，方便查看错误。

事件：
1、当某个组件发生了指定的动作后，会有相应的处理方案
2、事件源
3、事件：包括所有的事件源
4、监听器：每种监听器都有自己的工作职责
5、处理方式
6、动作监听器：对于鼠标点击和空格键都是起作用的。
7、鼠标监听器，若只想使用其中的一个方法，则可以使用适配器，该类已经实现了MouseListener接口，所有实现的方法都为空，比较特殊，为空实现
8、键盘监听器
...
3、蛇每迟到一个食物，就会长长一节
4、蛇咬到了石头或者咬到自己就会死亡
5、蛇开始的时候只有三个节点，而且出现在地图的正中央位置

6、食物可以随机的出现，但是食物不能出现在石头上，可以出现在蛇身上
7、地图由空地和石头

第一步：把下面的图形存储到一个容器上面，当把该容器的元素全部输出的时候，就出现该图形
***********************
*                     *
*                     *
*                     *
...
*                     *
***********************
使二维数组，char[][] background = new char[][];
二维数组是数组的数组，字符默认的值就是字符
蛇的节点是坐标而已，x和y
使用什么容器存储坐标	，linkedlist
食物就是个坐标
蛇的移动原理：添加一个新的蛇头，删除蛇尾巴
建立自己的代码库
解决蛇反方向行走的问题
如果需要画图，要使用到Jpanel
...
1、dos命令的交互方式
2、图形化界面的交互方式
3、在图形化界面编程中，所有的图形类都称为组件类
4、所有的图形类都位于java.awt和javax.swing
5、在awt中存在的图形类在swing一样存在，区别在于swing的图形类都以J开头
6、这两个包的图形类的区别：
	java.awt中的所有图形类的图形都是依赖系统的图形库的。而javax.swing中的是sun自己去实现的
7、java写出来的图形化界面比较丑，不推荐使用java开发桌面应用程序	
8、java面向互联网的应用，安全性非常高
9、所有的图形类都是组件	
	容器组件
	非容器组件
10、布局管理器用于摆放组件（类似于室内的设计者）
11、每种不同的布局管理器都有不同的风格
12、边框布局管理器（borderlayout）
13、流式布局管理器（一行一行的布局）
...
由网络层的IP协议和传输层的TCP协议组成。TCP/IP 定义了电子设备如何连入因特网，
以及数据如何在它们之间传输的标准。协议采用了4层的层级结构，每一层都呼叫它的下一层所提供的协议来完成自己的需求。
